...
theorem forall_ne [BI PROP] {Φ Ψ : α → PROP} (h : ∀ a, Φ a ≡{n}≡ Ψ a) :
  ...
  constructor <;> rintro Pa ⟨a, Hequiv⟩
  · exists (Ψ a); simp []; refine ⟨ ⟨a, BIBase.BiEntails.rfl⟩, ?_⟩
    replace Hequiv := Iris.BI.equiv_iff.2 Hequiv
    apply OFE.Dist.trans _ (h _)
    apply OFE.Dist.symm
    apply OFE.equiv_dist.1 Hequiv
  · exists (Φ a); simp []; refine ⟨ ⟨a, BIBase.BiEntails.rfl⟩, ?_⟩
    replace Hequiv := Iris.BI.equiv_iff.2 Hequiv
    apply OFE.Dist.trans (h _)
    apply OFE.equiv_dist.1 Hequiv

theorem forall_intro [BI PROP] {P : PROP} {Ψ : α → PROP} (h : ∀ a, P ⊢ Ψ a) : P ⊢ ∀ a, Ψ a :=
  sForall_intro fun _ ⟨_, eq⟩ => (h _).trans eq.1

theorem forall_elim [BI PROP] {Ψ : α → PROP} (a : α) : (∀ a, Ψ a) ⊢ Ψ a :=
  sForall_elim ⟨_, BIBase.BiEntails.rfl⟩
...
theorem exists_intro [BI PROP] {Ψ : α → PROP} (a : α) : Ψ a ⊢ ∃ a, Ψ a :=
  sExists_intro ⟨_, BIBase.BiEntails.rfl⟩

theorem exists_elim [BI PROP] {Φ : α → PROP} {Q : PROP} (h : ∀ a, Φ a ⊢ Q) : (∃ a, Φ a) ⊢ Q :=
  sExists_elim fun _ ⟨_, eq⟩ => eq.2.trans (h _)
...
theorem persistently_forall [BI PROP] [h : BIPersistentlyForall PROP] {Ψ : α → PROP} :
  ...
  rintro ⟨a, eq⟩;
  apply (forall_elim _).trans
  apply persistently_mono eq.1
...
theorem persistently_exists [BI PROP] {Ψ : α → PROP} : <pers> (∃ a, Ψ a) ⊣⊢ ∃ a, <pers> (Ψ a) := by
  ...
  refine exists_elim fun _ => pure_elim_l fun ⟨a, eq⟩ => ?_
  apply entails_trans.trans _ (exists_intro a)
  apply persistently_mono eq.2
...
instance forall_persistent [BI PROP] [BIPersistentlyForall PROP] (Ψ : α → PROP)
  ...
  sForall_persistent _ fun _ ⟨a, eq⟩ => by
  constructor
  apply entails_trans.trans eq.2
  cases (h a) with | mk h
  apply h.trans (persistently_mono eq.1)
...
instance exists_persistent [BI PROP] (Ψ : α → PROP) [h : ∀ x, Persistent (Ψ x)] :
    Persistent iprop(∃ x, Ψ x) := sExists_persistent _ fun _ ⟨a, eq⟩ => by
  constructor
  apply entails_trans.trans eq.2
  cases (h a) with | mk h
  apply h.trans (persistently_mono eq.1)
...
theorem later_forall_2 [BI PROP] {α} {Φ : α → PROP} : (∀ a, ▷ Φ a) ⊢ ▷ ∀ a, Φ a := by
  ...
  rintro ⟨a, Ha⟩
  refine imp_intro' <| and_elim_l.trans <| (forall_elim a).trans (later_mono Ha.1)
...
theorem later_exists_false [BI PROP] {Φ : α → PROP} :
  ...
  · refine or_intro_r' <| exists_elim ?_
    ...
    rintro ⟨a, eq⟩
    refine imp_intro' <| exists_intro' a (and_elim_l.trans (later_mono eq.2))
