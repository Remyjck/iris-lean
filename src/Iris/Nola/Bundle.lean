...
instance : Iris.IsCOFE.{u + 1} (aProp.{u} FF) where
  compl := fun c => by
    ...
    · sorry
    · sorry
  conv_compl {n c} := by simp [Iris.OFE.Dist]; sorry
...
protected def pure (P : Prop) : aProp FF := ⟨ false, AProp.pure P ⟩

protected def and (P Q : aProp FF) : aProp FF :=
  ⟨ P.tag || Q.tag, AProp.and P.car Q.car ⟩

protected def or (P Q : aProp FF) : aProp FF :=
  ⟨ P.tag || Q.tag, AProp.or P.car Q.car ⟩
...
protected def sep (P Q : aProp FF) : aProp FF :=
  ⟨ P.tag || Q.tag, AProp.sep P.car Q.car ⟩
...
theorem Dist_UPredDist {b₁ b₂} (P : AProp FF b₁) (Q : AProp FF b₂) :
  ...
  Iris.OFE.Dist (α := aProp FF) n { tag := b₁, car := P } { tag := b₂, car := Q } := by
  simp only [OFE.Dist]
